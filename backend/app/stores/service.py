...
async def get_stores(
    city: str | None, lat: float | None, lon: float | None, session: AsyncSession
) -> dict:
    ...
    if lat is not None and lon is not None:
        store_list.sort(
            key=lambda s: (
                calculate_distance(lat, lon, s['lat'], s['lon'])
                if s['lat'] and s['lon']
                else float('inf')
            )
        )
...
async def get_store_products(store_id: str, session: AsyncSession) -> dict:
    ...
    return {'store_id': store_id, 'products': product_list}
...
async def get_store_inventory(store_id: str, product_id: str, session: AsyncSession) -> dict:
    ...
    stmt = (
        ...
        .join(Store)
        .join(Product)
        .where(Store.uuid == store_id, Product.barcode == product_id)
    )
    result = await session.execute(stmt)
    row = result.first()
